...
class _ScannerScreenState extends ConsumerState<ScannerScreen> {
  ...
  Future<void> _onDetect(BarcodeCapture capture) async {
    ...
    if (state is ScannerFound) {
      await showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (_) => ProductFoundSheet(product: state.product));
    } else if (state is ScannerNotFound) {
      await showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (_) => const _ProductNotFoundSheet());
    } else if (state is ScannerError) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: ${(state as ScannerError).message}')));
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(l.productNotFound, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Text(l.productNotInDatabase, style: const TextStyle(color: Colors.grey)),
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            onPressed: () {
              ...
              showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (_) => const AddItemSheet());
            },
            child: Text(l.addManually),
          ),
        ),
        const SizedBox(height: 8),
